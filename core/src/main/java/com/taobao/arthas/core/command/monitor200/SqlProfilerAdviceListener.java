...
class SqlProfilerAdviceListener extends AdviceListenerAdapter {
    ...
    private static final MethodWithSignature PREPARED_STATEMENT_SET_PARAM_METHOD_MAP;
    private static final MethodWithSignature STATEMENT_EXECUTE_METHOD_MAP;
    private static final MethodWithSignature STATEMENT_EXECUTE_BATCH_METHOD_MAP;
    private static final MethodWithSignature STATEMENT_ADD_BATCH_METHOD_MAP;
    private static final MethodWithSignature STATEMENT_CLEAR_BATCH_METHOD_MAP;

    private static final MethodWithSignature PREPARED_STATEMENT_EXECUTE_METHOD_MAP;
    private static final MethodWithSignature PREPARED_STATEMENT_ADD_BATCH_METHOD_MAP;

    private final ThreadLocalWatch threadLocalWatch = new ThreadLocalWatch();
    private final SqlProfilerCommand command;
    private final CommandProcess process;

    // 缓存PreparedStatement对象对应的Sql语句
    private final Map<Statement, StatementSqlContext> statement2SqlMap = new ConcurrentWeakKeyHashMap<Statement, StatementSqlContext>();
    private final Map<PreparedStatement, PreparedStatementSqlContext> preparedStatement2SqlMap = new ConcurrentWeakKeyHashMap<PreparedStatement, PreparedStatementSqlContext>();

    // 用来解决connectionProxy、statementProxy会导致sql语句被记录多次的问题
    private final ThreadLocal<SqlExecuteStack> sqlStack = new ThreadLocal<SqlExecuteStack>();
    ...
    private class MonitorCollectTask implements Runnable {
        private final ConcurrentHashMap<String, SqlProfilerModel.SqlStat> sqlStatMap;
        private final CommandProcess process;
